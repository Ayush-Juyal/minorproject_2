...
public class AntColonyVmLoadBalancer extends VmLoadBalancer {
	...
	public void evaporatePheromones(double[][] pheromones, double evaporationRate) {
	...
	public void Evaporation() {
		evaporatePheromones(pheromones, EVAPORATION_FACTOR / 100.0);
	}
...
*/
	public class Ant {
		...
		public double[] computeProbability(int vmId) {
			...
			double sum = 0.0;
			double[] scores = new double[probability.length];

			// Calculate scores and sum
			for (int i = 0; i < probability.length; i++) {
				scores[i] = scoreFunction(vmId, i);
				sum += scores[i];
			}
	...
	public void UpdatePheromone(int prevId, int newId) {
		double deltaPheromone = ONE_UNIT_PHEROMONE;
		pheromones[prevId][newId] = (1 - EVAPORATION_FACTOR) * pheromones[prevId][newId] + deltaPheromone;
	}
